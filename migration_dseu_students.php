...
function registerUser($rno,$name,$mobile,$email,$father_name,$mother_name,$genderName,$slot_id,$section,$center_id,$batch_id){
		    $con = createConnection();
			...
			//// Now Adding  Admin Login 
			   $sql5 = "insert into user(first_name,email_id,address_id,profile_pic,updated_by,created_date,user_client_id,roll_no,fathers_name,mothers_name,section,slot,ex_center_id,ex_phone,ex_role_name,ex_user_group_id,ex_role_definition_id,ex_password,ex_loginid,gender) values('$name','$email','$address_id','$asset_id',1, NOW(),'$client_id','$rno','$father_name','$mother_name','$section','$slot_id','$center_id','$mobile','Learner','$client_group_id','2','$password','$rno','$genderId')";
			   $stmt = $con->prepare($sql5);	
			   $stmt->execute();
			   $user_id =$con->insert_id;
			   $stmt->close(); 		
     ...
     foreach ($slots_arr_count as $key => $value) {
       ...
       for ($i = 0; $i<=$indexCount; $i++) {
       	$movedStudents = moveElement($final_students,0,$sectionMaxCount);
			...
			$movedStudents[$k1]->{'rno'} = $mStu->{'roll_no'};
	   ...
	   for ($j = 1; $j<=$batchCount; $j++) {
		...
		foreach ($studentData2 as $k4 => $std2) {
			...
			foreach ($std2 as $k3 => $std) {
				...
				//registerUser($rno,$name,$mobile,$email,$father_name,$mother_name,$gender,$slot_id,$section,$center_id,$batch_id); //student migration code
