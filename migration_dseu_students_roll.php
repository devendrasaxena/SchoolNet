...
function getGenderIdByName($genderName){
	$con = createConnection();
    $sql = "Select id from tblx_gender where name='$genderName'";
...
/*$centerStr='39,40,41';
function updateUser1($center_ids){
	 $con = createConnection();
	 $userArr=array();
          $stmt = $con->prepare("Select user_id,ex_loginid from user WHERE ex_center_id IN($center_ids) and roll_no=''");
			$stmt->bind_result($user_id,$old_roll_no);
			$stmt->execute();
			while($stmt->fetch()) {
			 $userArr[]=$old_roll_no;
			}
...
$getUserIdArr=updateUser1($centerStr);
echo "count ";echo count($getUserIdArr);echo "<br> ";
echo "<pre>";print_r($getUserIdArr);
foreach($getUserIdArr as $key=>$value){
	$sql5 = "UPDATE user SET roll_no='$value' WHERE ex_loginid='$value' and  roll_no=''";
	$stmt = $con->prepare($sql5);	
	$stmt->execute();
	$stmt->close(); 	
}*/
function updateUser($rno,$roll_no,$genderName){
		    $con = createConnection();
          
			$genderId=getGenderIdByName($genderName);
			
			//$stmt = $con->prepare("Select user_id,ex_loginid from user WHERE ex_loginid='$rno' and roll_no=''");
			$stmt = $con->prepare("Select user_id,loginid from user_credential WHERE loginid='$rno'");
			$stmt->bind_result($user_id,$old_roll_no);
			$stmt->execute();
			$stmt->fetch();
			$stmt->close(); 
			//echo $user_id." - old rno ".$old_roll_no." == ".$rno." new ".$roll_no;
			//echo " <br> ";
			if(!empty($old_roll_no)){
				if($old_roll_no==$rno){
				echo "updatedb ".$user_id." - old rno ".$old_roll_no." == ".$rno." new ".$roll_no;
			   
				   $sql5 = "UPDATE user SET roll_no='$roll_no',gender='$genderId' WHERE user_id='$user_id'";
				   $stmt = $con->prepare($sql5);	
				  //$stmt->execute();
				   $stmt->close(); 	
				     echo " done ";
...
function createSlot($students){
    ...
    foreach ($students as $key => $student) {
    	...
    	if(!in_array($student->slot_code, $slots)){
    		...
    		$students_arr[$student->slot_code][] = $student->{'roll_no'};
     ...
     foreach ($slots_arr_count as $key => $value) {
       ...
       for ($i = 0; $i<=$indexCount; $i++) {
       	$movedStudents = moveElement($final_students,0,$sectionMaxCount);
			...
			$movedStudents[$k1]->{'roll_no'} = $mStu->{'roll_no'};
			...
			$movedStudents[$k1]->{'gender'} = $mStu->{'gender'};
	   ...
	   for ($j = 1; $j<=$batchCount; $j++) {
		...
		foreach ($studentData2 as $k4 => $std2) {
			...
			foreach ($std2 as $k3 => $std) {
				...
				updateUser($rno,$roll_no,$gender); //student update roll 
