	...
	if (move_uploaded_file($tmp_name, $pathtomove)) {
		...
		if ($userfile_extn == "xls") {
			...
			foreach ($grid as $index => $value) {
				...
				$id = $value[0];
				$roll_no = $value[1];
				$attendance_date = $value[2];
				$is_present = $value[3];

				$date = date('Y-m-d', strtotime(trim(addslashes(filter_string($attendance_date)))));

				$data_array = array();
				//$data['id'] = trim(addslashes(filter_string($id)));
				$data_array['roll_no'] = trim(addslashes(filter_string($roll_no)));
				$data_array['attendance_date'] = $date;
				$data_array['is_present'] = trim(addslashes(filter_string($is_present)));

				$status = $att->insertBulkAttendance($data_array);
				//print_r($status);
				if (!$status)
					$err++;
			}
					...
					<div class="row">
						<div class="panel panel-default bdrNone">
							<div class="panel-body padd20">
								<h3 class="panel-header"><?php echo $language[$_SESSION['language']]['bulk_upload_attendance']; ?></h3>
