...
//exit;
$batchData = get_data('SELECT batch_id FROM `tblx_batch_user_map` WHERE `user_id`=:user_id', array('user_id'=>$trainer_id));
$batch_ids = array();
foreach ($batchData as $key => $batch) {
	array_push($batch_ids, $batch->batch_id);
}
...
WHERE urm.role_definition_id = 2 AND tc.region = :region AND tc.center_id = :center_id 
AND tbum.batch_id IN($batch_ids)", array('region'=>$region_id, 'center_id'=>$center_id));



$csv_header = array('sno'=>'S. No','registration_id'=>'Registration No','roll_no'=>'Roll No','student_name'=>'Name of Student','gender'=>'Gender','fathers_name'=>'Father\'s Name','mothers_name'=>'Mother\'s Name','email_id'=>'Email ID','mobile'=>'Mobile Number','dseu_center_code'=>'Center ID','center_name'=>'Center Name','batch_name'=>'Slot Name','slot'=>'Slot','section'=>'Section','loginid'=>'User ID','teacher_name'=>'Name of Teacher','senior_teacher_name'=>'Name of Senior Teacher','time_spend'=>'Time spend in mins (App only)','pre_score'=>'Pre assessment ( App based numbers)','pre_level'=>'Pre assessment ( CEFR level)','post_score'=>'Post assessment ( App based numbers)','post_level'=>'Post assessment ( CEFR level)');
$head = 0;
...
foreach ($students as $key => $student) {
 ...
 WHERE ucm.center_id = :center_id AND urm.role_definition_id = 1 AND tbum.batch_id=:batch_id", array('center_id'=>$student->center_id,'batch_id'=>$student->batch_id),1);
	...
	$pre_placement = get_data("SELECT score FROM tblx_placement_result WHERE exam_type = 'pre' AND user_id =:user_id", array('user_id'=>$user_id),1);
	
	$post_placement = get_data("SELECT score FROM tblx_placement_result WHERE exam_type = 'post' AND user_id =:user_id", array('user_id'=>$user_id),1);
	...
	$pre_level = get_data("SELECT roll_no, level_value FROM dseu_prepost WHERE exam_type = 'pre' AND roll_no = :roll_no", array('roll_no'=>$student->roll_no),1);
	
	$post_level = get_data("SELECT roll_no, level_value FROM dseu_prepost WHERE exam_type = 'post' AND roll_no = :roll_no", array('roll_no'=>$student->roll_no),1);
            ...
            $i = 0;
             ...
             foreach($students  as $key3 => $value){
               ...
               foreach ($csv_header as $field_name => $val) {
             				  ...
             				  $export_data .= "<td>".$value->{$field_name}."</td>";
             			}
