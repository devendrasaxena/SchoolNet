...
$adminObj = new centerAdminController();
 ...
 if((($roleID==1) && ($trainer_limit>$chkLimit)) || ($roleID==2) && ($student_limit>$chkLimit)){					
	...
	if(empty($userIdVal) && ($_POST['uSignUp'] == 'studentReg') ) {
	...
	}else if(empty($userIdVal) && ($_POST['uSignUp'] == 'teacherReg') ) {
		...
		if($res){
			if(isset($res['user_id']) && $res['user_id']!='' && $res['loginid']!=''){
				// case : user is already registered
				 $_SESSION['error'] = 2;
				 $_SESSION['msg'] ="$teacher login id is already exist. Please try another.";
				 header("Location: ../createTeacher.php");
				 exit;
			 }else{

					$centerAdminName=filter_string($_POST['name']);
					$email_id=filter_string($_POST['email']);
					$password=filter_string($_POST['password']);	
			...
			/* if($is_teacher=='Yes'){
				$_SESSION['succ'] = 1;
				$_SESSION['msg'] ="$teacher created successfully.";
				header("Location: ../teacherAllList.php");
			}
			else{
				$_SESSION['succ'] = 1;
				$_SESSION['msg'] ="$teacher created successfully.";
		  ...
		  exit; */
			...
			$_SESSION['error'] = 1;
			$_SESSION['msg'] ="$teacher not saved. Please try again.";
			header("Location: ../createTeacher.php");
			exit;
		}
	}
	...
	if($roleID==2){
		...
		$_SESSION['error'] = 3;
		$_SESSION['msg'] ="You have reached maximum limit of $teacher allowed. You can not register more $teacher.";
		header("Location: ../createTeacher.php");
		exit;}		
	
} 
// For Edit /update
if(isset($userIdVal) && $userIdVal != '' && $_POST['uSignUp'] == 'teacherReg'){

	$userUpdate =$centerObj->updateUser($_POST);	
	...
	if($userUpdate){
		...
		if($is_teacher=='Yes'){
			$_SESSION['succ'] = 2;
			$_SESSION['msg'] ="$teacher not updated. Please try again.";
			header("Location: ../teacherAllList.php");
		}
		else{
			$_SESSION['succ'] = 2;
