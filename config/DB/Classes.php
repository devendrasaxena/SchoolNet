...
class ModelDB{
    ...
    public function __construct($db='LOCAL') {
        $this->pdo = DBConnection::createConn();
    }
...
class DB extends ModelDB{
   ...
   public function get($type = 'obj'){
    ...
    if($type == 'obj'):
        ...
        if(!$this->where)
         ...
         $data = $this->fetch_all_assoc($this->table,$this->select,$this->join.$this->where.$this->group.$this->order.$this->limit); 
   ...
   public function first($type='obj'){
    ...
    if($type == 'obj'):
        if(!$this->where)
            ...
            $data = $this->fetch_one_assoc($this->table,$this->select,$this->join.$this->where.$this->group.$this->order.$this->limit);
     endif;
   ...
   public function count($column = '*'){
    ...
    if(!$this->where)
        ...
        $data = $this->fetch_one_object($this->table,'COUNT('.$column.') AS total',$this->join.$this->where.$this->group.$this->order.$this->limit); 
