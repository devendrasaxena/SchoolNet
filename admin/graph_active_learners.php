...
$active_users = $graphObj->getActiveUsers($options);
 ...
 <section class="padder reportList"> 
	 ...
	 <?php // if(count($all_center_list_arr) > 0){?>
		<form id="serachform" name="serachform" method="GET" 
		 class="form-horizontal form-centerReg"
		  action="graph_active_learners.php" >
	<section class="marginBottom5 serachformDiv" >
		...
		<div class="col-xs-3 col-sm-3 col-md-3 col-lg-6 text-left paddLeft0">
		 ...
		 <div id="startDate" class="input-append date form-control input-lg" style="position: relative;width: 180px;display: inline-block;">
				<input  data-date-format="YYYY-MM-DD" readonly="true"  name="date" id="start_date" placeholder="YYYY-MM-DD" class=" width100per bdrNone" autocomplete="off" tabindex="4" value='<?php echo $selectedDate?>' style="width: 120px;" />
			...
			<div class="col-xs-3 col-sm-3 col-md-3 col-lg-3 text-right paddRight0">
				<button type="submit"  class="btn btn-red" id="btnSave"  title=" <?php echo $language[$_SESSION['language']]['search'] ?> <?php echo $language[$_SESSION['language']]['active_learners'] ?>"
				 style="margin-top:0px"> 
				 <?php echo $language[$_SESSION['language']]['search']; ?></button>
				
				<a class="btn btn-sm btn-red btnwidth40" href="graph_active_learners.php" name="refresh" title=" <?php echo $language[$_SESSION['language']]['refresh']; ?> " style="margin-top:0px"> <i class="fa fa-refresh"></i></a>
			</div>
...
var stdate = '<?php echo $selectedDate; ?>';
var date = new Date();
