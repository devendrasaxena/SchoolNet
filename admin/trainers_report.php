...
//$users_arr=$reportObj->getUsersByCenter('',1);
...
if (isset($_REQUEST['report_type'])) {
    ...
    if ($_REQUEST['report_type'] == 'export') {
        ob_clean();
        $file = 'trainers_report_'.time().'.csv';
 ...
 <section class="padder reportList"> 
	<?php include_once('reports_menu.php');?>
	  <div class="tab-content">
		...
		<form id="serachform" name="serachform"  method = "GET"  class="form-horizontal form-centerReg" action="trainers_report.php" >
			...
			<div class="col-xs-3 col-sm-3 col-md-3 col-lg-3 text-left paddLeft0">
					<select name="center_id" id="center_id" class="form-control " >
					   ...
					   $center_list_arr_drop_down =$reportObj->getCenterListByClient($client_id,'',$country);
						...
						echo '<option value="">'. $language[$_SESSION['language']]['states'] .'</option>';
						}
			...
			<div class="col-xs-3 col-sm-3 col-md-3 col-lg-3 text-right paddRight0 text-right paddRight0">
				...
				<a class="btn btn-sm btn-red btnwidth40" href="trainers_report.php" name="refresh" title=" <?php echo $language[$_SESSION['language']]['refresh']; ?>" style="margin-top:0px"> <i class="fa fa-refresh"></i></a>
			 </div>
			</form>
		  ...
		  <table class="table table-border dataTable table-fixed">
			...
			<thead  class="fixedHeader">
			  ...
			  <th class="col-sm-2 text-left textUpper">
				  <a href="trainers_report.php?country=<?php echo $country;?>&center_id=<?php echo $center_id;?>&batch_id=<?php echo $batch_id;?>&sort=u1.first_name&dir=<?php echo $dir?>" class="th-sortable"><?php echo $language[$_SESSION['language']]['name']; ?>
						<span class="th-sort"> 
							<?php 
			   ...
			   <th class="col-sm-3 text-left textUpper"><a href="trainers_report.php?country=<?php echo $country;?>&center_id=<?php echo $center_id;?>&batch_id=<?php echo $batch_id;?>&sort=u1.email_id&dir=<?php echo $dir?>" class="th-sortable"><?php echo $language[$_SESSION['language']]['login_id']; ?>
						<span class="th-sort"> 
							<?php 
			   ...
			   <th class="col-sm-2 text-left textUpper"><a href="trainers_report.php?country=<?php echo $country;?>&center_id=<?php echo $center_id;?>&batch_id=<?php echo $batch_id;?>&sort=uc1.created_date&dir=<?php echo $dir?>" class="th-sortable"><?php echo $language[$_SESSION['language']]['joined']; ?>
						<span class="th-sort"> 
							<?php 
		   ...
		   <tbody>	
			...
			<?php //}  
			  ...
			  <?php } else{   ?>
			<div class="col-xs-12 noRecord text-center"><?php echo $language[$_SESSION['language']]['records_not_available.']; ?></div>
		  <?php }?>  
			</tbody>
		    </table>
			</div>
...
<script>
        ...
        $(document).ready(function () {
             $(".export-report").click(function(e){
                e.preventDefault();
                var url = 'trainers_report.php?report_type=export';
