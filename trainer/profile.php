...
$adminData = $adminObj->getCenterAdminDataByID($_SESSION['user_id'],1); // trainer ole 1
...
if(isset($_SESSION['error']) && $_SESSION['error'] != ""){
	if($_SESSION['error'] == '1'){
		$msg = "Profile not saved. Please try again.";
	}
}
         ...
         <div class="panel-body">
		   <div class="col-sm-4 col-xs-4">
				...
				<?php if($adminData->system_name != ''){ ?>
							<div class="thumb-md thumb-md-logo text-center  fileInputs buttonImg relative" id="logoImg" >
						...
						<span class="dataImgShow uploadIcon">  <a  href="javascript:void(0)" class="editInputs buttonImg pointer" id="editProfileImg" onclick="uploadFile(this.id,'fileProfile','fileImgProfile','fileImgNamePro','viewImgProfile','profile');" style="cursor:pointer"> <i class="fa fa-edit"></i> </a></span>
							...
							<span class="dataImgShow uploadIcon">  <a  href="javascript:void(0)" class="editInputs buttonImg pointer" id="editProfileImg" onclick="uploadFile(this.id,'fileProfile','fileImgProfile','fileImgNamePro','viewImgProfile','profile');" style="cursor:pointer"> <i class="fa fa-edit"></i> Edit</a>
								...
								<?php }else{ ?>
								<div class="thumb-md thumb-md-logo text-center fileInputs buttonImg relative" id="editProfileImg">
		  ...
		  <div class="form-group col-sm-8">
		   <div class="form-group col-sm-12">
			  ...
			  <input type="text" name="name" id="name" placeholder="Name" class="form-control input-lg "  data-required="true" value="<?php echo $adminData->first_name; ?>" maxlength = "30"  autocomplete="nope"/>
             </div>
			<div class="clear"></div>
		   <div class="form-group col-sm-6">
			...
			<div class="clear"></div>
		  <div class="form-group col-sm-6">
                  <label for="qualification" class="control-label"><?php echo $language[$_SESSION['language']]['qualification']; ?> <span class="required"></span></label>
				   <select class="form-control input-lg parsley-validated fld_class" name="qualification" id="qualification">
				 <option value="" > <?php echo $language[$_SESSION['language']]['select'].' '.$language[$_SESSION['language']]['qualification']; ?></option>
				  <?php foreach($education as $key => $educationValue){
				
						 $selectedEducation =  (  $adminData->qualification == $educationValue['id'] ) ?  'selected ="selected"' : '';
			...
			<div class="form-group  col-sm-6" style="display:none">
                  <label for="business_unit" class="control-label"><?php echo $language[$_SESSION['language']]['business_unit']; ?> <span class="required"></span></label>
                  <input type="text" placeholder = "Enter business unit" class="form-control greenBorder" id="business_unit" name="business_unit"  value="<?php echo $adminData->business_unit; ?>"/>
                </div>
			</div>
			<div class="clear"></div>
				<div  class="col-sm-12">
	 	 ...
	 	 <a href='dashboard.php' class="btn btn-primary "><?php echo $language[$_SESSION['language']]['cancel']; ?></a>&nbsp;&nbsp;
	        <input id="profile_id" type="hidden" name="profile_id" value="<?php echo $adminData->profile_id; ?>"/>
				 ...
				 <input id="cid" type="hidden" name="cid" value="<?php echo $center_id; ?>"/>
				 <input type="hidden" id="cpFlag" value="0" />
			  <button type="submit" name="uSignUp" value='trainerReg' class="btn btn-s-md btn-primary"><?php echo $language[$_SESSION['language']]['submit']; ?> </button>
