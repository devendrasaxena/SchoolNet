...
<div class="breadcrumbBgNone breadcrumbPadder">
	<div class="col-md-6 col-sm-6 text-left paddLeft0"><?php echo $language[$_SESSION['language']]['learners']; ?> </div>
	<div class="col-md-6 col-sm-6 text-right paddRight0"><span class="pull-right">
	<?php if($editRight=='1'){?><a href='createStudent.php' title=" <?php echo $language[$_SESSION['language']]['add_learner'] ?>" class="btn btn-primary marginTop0"><?php echo $language[$_SESSION['language']]['add_learner']; ?></a>
	 <a href='bulkStudentUpload.php' title=" <?php echo $language[$_SESSION['language']]['bulk_upload_learners'] ?>" class="btn btn-primary marginTop0"><?php echo $language[$_SESSION['language']]['bulk_upload_learners']; ?> </a>
	<?php }?>
	<?php if($region_id=='5'){?>
	<a href='bulkAttendanceUpload.php' title=" <?php echo $language[$_SESSION['language']]['bulk_upload_attendance'] ?>" class="btn btn-primary marginTop0"><?php echo $language[$_SESSION['language']]['bulk_upload_attendance']; ?> </a>
	<a href='bulkprepostAssessment.php' title="<?php echo $language[$_SESSION['language']]['upload_test_data'] ?>" class="btn btn-primary marginTop0"><?php echo $language[$_SESSION['language']]['upload_test_data']; ?> </a>
	<?php }?>
</span>	</div>
 </div>
 <div class="clear"></div>
 ...
 <form id="serachform" name="serachform"  method = "get"  class="form-horizontal form-centerReg" data-validate="parsley" onSubmit="return confSubmit();" action="studentList.php" >
	<section class="marginBottom5 serachformDiv">
       ...
       <div class="col-xs-4 col-sm-4 col-md-4 col-lg-4 text-left paddLeft0 <?php if($_SESSION['role_id']==7){?> hide <?php }?>" >

		 <select name="region_id" id="region" class="form-control "  >
			<option value=""><option value=""><?php echo $language[$_SESSION['language']]['select_state']; ?></option>
			  ...
			  <?php 
				...
				<div class="col-xs-3 col-sm-3 col-md-3 col-lg-3 text-left paddLeft0">
							<select name="center_id" id="center_id" class="form-control" >
								<?php $optiondisabled = ($center_id ) ? "disabled" : ""; ?>
			...
			<div class="clear" style="margin-top:10px;">&nbsp;</div>
			 ...
			 <div class="col-xs-5 col-sm-5 col-md-5 col-lg-5 pull-right text-right padd0">
					<button type="submit" name="Submit" class="btn btn-red" title="<?php echo $language[$_SESSION['language']]['learners']; ?> <?php echo $language[$_SESSION['language']]['search']; ?>" id="btnSave" style="margin-top:0px"> <?php echo $language[$_SESSION['language']]['search']; ?></button>
  ...
  <section class="panel panel-default">
     ...
     <div class="panel-body">
		   ...
		   <thead  class="fixedHeader">
			 ...
			 <th class="col-sm-2 text-left paddLeft0 paddRight0"><?php echo $language[$_SESSION['language']]['states']; ?><a href="studentList.php?country=<?php echo $country; ?>&center_id=<?php echo $center_id; ?>&batch_id=<?php echo $batch_id; ?>&student=<?php echo $student_id; ?>&student_txt=<?php echo filter_query($_REQUEST['student_txt']); ?>&course_id=<?php echo $course_id; ?>&sort=name&dir=<?php echo $dir; ?>" class="th-sortable hide"> 
			 ...
			 <th class="col-sm-1 text-left hide"><?php echo $language[$_SESSION['language']]['user_type']; ?></th>
			  <th class="col-sm-1 text-left"><?php echo $language[$_SESSION['language']]['status']; ?></th>
			  <th class="col-sm-1 text-left hide" ><?php echo $language[$_SESSION['language']]['expiry_date']; ?><!--<a href="studentList.php?country=<?php echo $country; ?>&center_id=<?php echo $center_id; ?>&batch_id=<?php echo $batch_id; ?>&student=<?php echo $student_id; ?>&student_txt=<?php echo filter_query($_REQUEST['student_txt']); ?>&course_id=<?php echo $course_id; ?>&sort=expiry_date&dir=<?php echo $dir; ?>" class="th-sortable">Expiry Date<span class="th-sort"> 
					<?php 
			...
			<?php
			 foreach($users_arr  as $key => $value){
				...
				<td class="col-sm-3 text-left"><?php echo $email_id; ?></td>
				  ...
				  <td class="col-sm-1 text-left hide"><?php echo strtoupper($value->user_from); ?></td>
				   <td class="col-sm-1 text-left" <?php echo $activeClass;?>><?php echo $status; ?></td>
				   <td class="col-sm-1 text-left  hide"><?php echo $expiryDate; ?></td>
				   <td class="col-sm-2 text-center">
				  <?php if($edit_action=='1'){?> <a class="edit" title="<?php echo $language[$_SESSION['language']]['edit']; ?>" href="<?php echo "createStudent.php?uid=".base64_encode($value->user_id); ?>"> <i class="fa fa-edit"> </i> <?php echo $language[$_SESSION['language']]['edit']; ?></a>
				  |<?php }?>
				   <a title="<?php echo $language[$_SESSION['language']]['view_details']; ?>" href="learner-detail.php?uid=<?php echo $uid?>">  <i class="fa fa-eye"> </i> <?php echo $language[$_SESSION['language']]['view_details']; ?> </a>
			 ...
			 <?php }else{ ?>
				...
				<?php if($editRight=='1'){?><br> Click "<span class="capitalize"><?php echo $language[$_SESSION['language']]['add_learner']; ?>" </span> to <span class="textLower"> <?php echo $language[$_SESSION['language']]['add_learner'];?></span><?php }?>
				</div><?php } ?>
...
$(document).ready(function(){
    $('.search-box input[type="text"]').on("keyup input", function(){
		...
		if(inputVal.length && inputVal.length>0){
            $.post("ajax/search_student.php", {uname: inputVal,batch_id: batch_id,center_id: center_id,country: country,status: status,region_id: region_id,role_id: 2}).done(function(data){ 
