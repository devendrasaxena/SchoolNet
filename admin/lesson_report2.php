...
if($student_id=='All' || $student_id=='0' || $student_id=='' || $student_id=='undefined'){
	...
	//$request_param = "country_name=".$country_req."&batch_id=".$batch_id_req."&inst_id=".$center_id_req."&user_id=".$student_id_req."&start=".$objPage->_db_start."&limit=".$_limit."&sort=user_name%20asc";
	 ...
	 if(isset($_REQUEST['report_type']) && $_REQUEST['report_type'] == 'export') {
		...
		$request_param = "country_name=".$country_req."&batch_id=".$batch_id_req."&inst_id=".$center_id_req."&user_id=".$student_id_req."&topic_type=1&record_type=chapter&limit=35000&sort=".$order."%20".$dir; 
		...
		$request_param = "country_name=".$country_req."&batch_id=".$batch_id_req."&inst_id=".$center_id_req."&user_id=".$student_id_req."&topic_type=1&record_type=chapter&start=".$start_page."&limit=".$_limit."&sort=".$order."%20".$dir;
...
if (isset($_REQUEST['report_type'])) {
	...
	//	if($total_response > 0){
			foreach($res['response']['docs']  as $key => $value){
				...
				//$last_attempted_date ='';
				//if($value['quiz_last_attempted_date'])
				$last_attempted_date = !empty($value['quiz_last_attempted_date'])? date("d-m-Y H:i", strtotime($value['quiz_last_attempted_date'])) : '-' ; 	
				//$last_attempted_date = date("d-m-Y H:i", strtotime($value['quiz_last_attempted_date']));	
				if($last_attempted_date=='01-01-1970'){
					$last_attempted_date = '-';			
				}
				
				//	$answer_exp_submit = '';
				
				//$topic_name = !empty($value['topic_name'])?$value['topic_name']:'-';
				//$user_name = !empty($value['user_name'])?$value['user_name']:'-';
			//	$course = !empty($value['course'])?$value['course']:'';
			//	$chapter_name = !empty($value['chapter_name'])?$value['chapter_name']:'-';
				$chapter_attempt = !empty($value['chapter_attempt'])?$value['chapter_attempt']:'-';
				$quiz_score = !empty($value['quiz_score'])?$value['quiz_score']:'-';
			
				
				/*
				if(isset($value['topic_name'])){
					$topic_name = $value['topic_name']; 
					
				}else{
				 $topic_name = 'ttttttttttt'; 	
				}
				...
				*/
                ...
                /*
					...
					$export_data .= '<th>' . $user_name.'</th>';
					$export_data .= '<th>' . $value['course']. '</th>';
					//$export_data .= '<th>' . $value['topic_name'] . '</th>';
					$export_data .= '<th>' . $topic_name . '</th>';
					$export_data .= '<th>' . $value['chapter_name']. '</th>';					
					$export_data .= '<th>' . $value['chapter_attempt'].'</th>';
					$export_data .= '<th>' . $last_attempted_date. '</th>';
					$export_data .= '<th>' . $value['quiz_score']. '</th>';
				*/	
				    $i++;
				    $export_data .= '<tr>';
					$export_data .= '<th>' . $i . '</th>';
					...
					//$export_data .= '<th>' . $user_name.'</th>';
					//$export_data .= '<th>' . $course. '</th>';
					//$export_data .= '<th>' . $value['topic_name'] . '</th>';
					//$export_data .= '<th>' . $topic_name . '</th>';
					//$export_data .= '<th>' . $chapter_name. '</th>';					
					$export_data .= '<th>' . $chapter_attempt.'</th>';
					$export_data .= '<th>' . $last_attempted_date. '</th>';
					$export_data .= '<th>' . $quiz_score. '</th>';
...
?>
 ...
 <ul class="nav nav-tabs" style="margin-bottom:40px;">
	...
	<li class="textUpper"><a  class="" href="time_spent_report.php">Time Spent Report</a></li>
  </ul>
...
<script>
        ...
        $(document).ready(function () {
             $(".export-report").click(function(e){
                e.preventDefault();
                var url = 'lesson_report2.php?report_type=export';
...
$(document).ready(function(){
    $('.search-box input[type="text"]').on("keyup input", function(){
	   ...
	   if(inputVal.length && inputVal.length>0){
			$.post("ajax/search_student_curl.php", {uname: inputVal,batch_id: batch_id,center_id: center_id,country: country}).done(function(data){
