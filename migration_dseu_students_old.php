...
function createBatch($batchName,$center_id){
	$con = createConnection();
	...
	if(empty($batch_id)){
		...
		echo $batch_id." not empty";//exit;
		echo $batch_code = "CN-".$center_id.'-B'.$batch_id;
...
echo '<pre>';
  ...
  //echo '<pre>';print_r($moveStudentArr);
   return ['students_data'=>$allStudents,'moveStudent'=>$moveStudentArr];
}
...
if($result->status=='200'){
	...
	foreach ($batchData as $k2 => $btd) {
       ...
       //echo '<pre>';print_r($batchData1->slot_code);exit;
       /* 
	      //Single insert Test single batch
	      $batchName="Slot-".$btd[0]->slot_code;
		  $center_id=$btd[0]->center_id;
		 createBatch($batchName,$center_id); //batch migration code
	   */
		...
		/*
     		//Single insert Test single user
			$rno=$batchData1->rno; 
			$mobile=$batchData1->mobile ;
			$name=$batchData1->name;
			$email=$batchData1->email_id;
			$father_name=$batchData1->father_name;
			$mother_name=$batchData1->mother_name;
			$slot_id=$batchData1->slot_id;
			$section=$batchData1->section;
			$center_id=$batchData1->center_id;
			$batch_id=$batchData1->batch_id;
			//registerUser($rno,$name,$mobile,$email,$father_name,$mother_name,$slot_id,$section,$center_id,$batch_id);
			//echo '<pre>';print_r($batchData1);//exit;
	   */
		//echo count($studentData1[$batchName]);
		//echo '<pre>';print_r($studentData1[$batchName]);//exit;
		foreach ($studentData1[$batchName] as $k3 => $std) {
			//echo '<pre>';print_r($std->rno);
			$rno=$std->rno;
			$mobile=$std->mobile ;
			$name=$std->name;
			$email=$std->email_id;
