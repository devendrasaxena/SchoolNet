...
if(count($getProductData)==1){
   ...
   if(SHOW_PRE_EXAM==1){
	    //echo "<pre>";print_r($_SESSION);exit;
		   ...
		   if($testTrackInfo['attempted']=='yes'){
			   $level_assigned=$testTrackInfo['level_assigned'];
			   $testUrl='';
			   $showTest=0;
			//  header('location:courseTest.php');
			  //exit; 
			   ...
			   if(isset($_SESSION['is_skip']) && $_SESSION['is_skip']==1){
				...
				$testUrl=''; 
			     ...
			     $testUrl=$globalLink.'/user/pTest.php?user_id='.base64_encode($_SESSION['user_id']);//$testTrackInfo['placement_url'];  
 ...
 <?php 
   ...
   if(SHOW_PRE_EXAM==1){
	...
	//}else{
	 ...
	 ?>
     <li class="col-md-4 text-center slideContentTopic item chap-asmt-box" pid="<?php echo $productInfo['id'];?>" ><a id="pro<?php echo $i;?>" href="javascript:void(0)" path="<?php echo $productPath;?>" onclick="visitProduct('<?php echo $i;?>','<?php echo $batchId;?>','<?php echo $productInfo['id'];?>','<?php echo $productPath;?>','<?php echo $package_code;?>','<?php echo $showTest;?>','<?php echo $level_assigned;?>')">
	   <div class="relative">
